...
module Custom = struct
  ...
  let create_compare compare n info = make compare (create n info)
  let create = create_compare
  let clear t = clear t.graph

  let size_vertex t = size_vertex t.graph
  let size_hedge t = size_hedge t.graph
  let size_edgevh t = size_edgevh t.graph
  let size_edgehv t = size_edgehv t.graph
  let size t = size t.graph

  let attrvertex t v = Compare.attrvertex t.compare t.graph v
  let attrhedge t h = Compare.attrhedge t.compare t.graph h
  let info t = info t.graph

  let is_vertex t v = Compare.is_vertex t.compare t.graph v
  let is_hedge t h =  Compare.is_hedge t.compare t.graph h
  let is_empty t = is_empty t.graph

  let succhedge t v = Sette.Custom.make t.compare.compareh (Compare.succhedge t.compare t.graph v)
  let predhedge t v = Sette.Custom.make t.compare.compareh (Compare.predhedge t.compare t.graph v)
  let succvertex t h = Compare.succvertex t.compare t.graph h
  let predvertex t h = Compare.predvertex t.compare t.graph h
  let succ_vertex t v = Sette.Custom.make t.compare.comparev (Compare.succ_vertex t.compare t.graph v)
  let pred_vertex t v = Sette.Custom.make t.compare.comparev (Compare.pred_vertex t.compare t.graph v)

  let add_vertex t v attrvertex =
    Compare.add_vertex t.compare t.graph v attrvertex
  let add_hedge t h attrhedge ~pred ~succ =
    Compare.add_hedge t.compare t.graph h attrhedge ~pred ~succ

  let replace_attrvertex t v attrvertex =
    Compare.replace_attrvertex t.compare t.graph v attrvertex
  let replace_attrhedge t h attrhedge =
    Compare.replace_attrhedge t.compare t.graph h attrhedge

  let remove_hedge t h = Compare.remove_hedge t.compare t.graph h
  let remove_vertex t v = Compare.remove_vertex t.compare t.graph v

  let iter_vertex t f =
    Hashhe.iter
      (fun v vertex_n -> f v vertex_n.attrvertex ~pred:(Sette.Custom.make t.compare.compareh vertex_n.predhedge) ~succ:(Sette.Custom.make t.compare.compareh vertex_n.succhedge))
      t.graph.vertex
  let iter_hedge t f =
    Hashhe.iter
      (fun n hedge_n -> f n hedge_n.attrhedge ~pred:hedge_n.predvertex ~succ:hedge_n.succvertex)
  ...
  let fold_vertex t f res =
    Hashhe.fold
      (fun v vertex_n res -> f v vertex_n.attrvertex ~pred:(Sette.Custom.make t.compare.compareh vertex_n.predhedge) ~succ:(Sette.Custom.make t.compare.compareh vertex_n.succhedge) res)
      t.graph.vertex res
  ...
  let topological_sort ?priority t root =
    Compare.topological_sort t.compare ?priority t.graph root

  let topological_sort_multi vertex_dummy hedge_dummy ?priority t root =
    assert(t.compare.comparev==root.Sette.Custom.compare);
    Compare.topological_sort_multi t.compare vertex_dummy hedge_dummy ?priority t.graph root.Sette.Custom.set

  let reachable ?filter t root =
    let (setv,seth) =
      Compare.reachable t.compare ?filter t.graph root
    in
    (Sette.Custom.make t.compare.comparev setv,
    Sette.Custom.make t.compare.compareh seth)

  let reachable_multi vertex_dummy hedge_dummy ?filter t root =
    assert(t.compare.comparev==root.Sette.Custom.compare);
    let (setv,seth) =
      Compare.reachable_multi t.compare vertex_dummy hedge_dummy ?filter t.graph root.Sette.Custom.set
    in
    (Sette.Custom.make t.compare.comparev setv,
    Sette.Custom.make t.compare.compareh seth)

  let cfc ?priority t root =
    Compare.cfc t.compare ?priority t.graph root

  let cfc_multi vertex_dummy hedge_dummy ?priority t root =
    assert(t.compare.comparev==root.Sette.Custom.compare);
    Compare.cfc_multi t.compare vertex_dummy hedge_dummy ?priority t.graph root.Sette.Custom.set

  let scfc ?priority t root =
    Compare.scfc t.compare ?priority t.graph root

  let scfc_multi vertex_dummy hedge_dummy ?priority t root =
    assert(t.compare.comparev==root.Sette.Custom.compare);
    Compare.scfc_multi t.compare vertex_dummy hedge_dummy ?priority t.graph root.Sette.Custom.set

  let print pv ph pattrv pattrh pinfo fmt t =
    Compare.print t.compare pv ph pattrv pattrh pinfo fmt t.graph
  ...
  let min t = Sette.Custom.make t.compare.comparev (Compare.min t.compare t.graph)
  let max t = Sette.Custom.make t.compare.comparev (Compare.max t.compare t.graph)
