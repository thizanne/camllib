...
module Custom = struct
  ...
  let is_empty t = is_empty t.set
  let singleton compare x = make compare (singleton x)
  let iter f t = iter f t.set
  let fold f t acc = fold f t.set acc
  let for_all f t = for_all f t.set
  let exists f t = exists f t.set
  let cardinal t = cardinal t.set
  let elements t = elements t.set
  let min_elt t = min_elt t.set
  let max_elt t = max_elt t.set
  let choose = min_elt
  let print ?first ?sep ?last px fmt t = print ?first ?sep ?last px fmt t.set
  let add x t = make t.compare (Compare.add t.compare x t.set)
  let mem x t = Compare.mem t.compare x t.set
  let remove x t = make t.compare (Compare.remove t.compare x t.set)
  let union t1 t2 = 
    assert(t1.compare==t2.compare);
    make t1.compare (Compare.union t1.compare t1.set t2.set)
  let inter t1 t2 = 
    assert(t1.compare==t2.compare);
    make t1.compare (Compare.inter t1.compare t1.set t2.set)
  let diff t1 t2 = 
    assert(t1.compare==t2.compare);
    make t1.compare (Compare.diff t1.compare t1.set t2.set)
  let equal t1 t2 =
    assert(t1.compare==t2.compare);
    Compare.equal t1.compare t1.set t2.set
  let compare t1 t2 = 
    assert(t1.compare==t2.compare);
    Compare.compare t1.compare t1.set t2.set
  let subset t1 t2 =
    assert(t1.compare==t2.compare);
    Compare.subset t1.compare t1.set t2.set
  let filter f t = 
    make t.compare (Compare.filter t.compare f t.set)
  let partition f t = 
    let (set1,set2) = Compare.partition t.compare f t.set in
